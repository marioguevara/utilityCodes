...
###PREDICTION EXAMPLE\
##make predictions across any point in the ground
#perform a prediction map
#build a map from model with optimal parameters
map_rf <- predict(predictor_stack_test[[names(x)]], results )
test$rf <-  predict(results, test[names(x)])


# predict values for test data
ens$rf <- predict(results, ens[1:15])
...
#get the residual
ens$residual <- ens@data$SOC1 - expm1(ens@data$rf)
...
#save the maps
writeRaster(map_rf, file='output/RF_prediction.tif', overwrite=TRUE )
writeRaster(kr_map, file='output/RF_OK_residuals.tif', overwrite=TRUE )
#
