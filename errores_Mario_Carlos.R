...
dat <- dat[,c("promAltura","bh","alos","canopy","asar","X","Y")]

# quantile random forest model (Warning: quantile rf is slow)
quant_rf_model <- quantregForest(y=training$Y, x=training[,2:13])
...
rf_model <- randomForest(y=dat$promAltura, x=dat[,2:7],ntree=1000,nodesize=10)

# OOB absolut error
oob_abs <- abs(dat$promAltura-rf_model$predicted) # absolute difference

# associate uncertainty with OOB squared error
oob_abs_database <- data.frame(oob_abs,X=dat$X,Y=dat$Y)
var_1sd_database <- data.frame(var_1sd,X=region$X,Y=region$Y)
...
#### lim_sup
Lim_sup_1sd_database <- data.frame(Lim_sup_1sd,X=region$X,Y=region$Y)
...
#### lim_inf
Lim_inf_1sd_database <- data.frame(Lim_inf_1sd,X=region$X,Y=region$Y)
...
#### Original
original_database <- data.frame(predict(rf_model,region),X=region$X,Y=region$Y)
